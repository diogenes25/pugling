...
| Zweck | Route |
|-------|-------|
| Vater: Lehrplan CRUD + Vokabeln | `POST/GET/PATCH /api/study-plans`, `…/{id}/items` |
| Vater: Tag-für-Tag-Fortschritt | `GET /api/study-plans/{id}/progress` |
| Vater/Sohn: Ein-Blick-Status heute | `GET /api/study-plans/{id}/today` |
| Vater: Lern-Doku + Testhistorie | `GET /api/study-plans/{id}/report` |
...
- **`ClozeText`-Store** (Lerngrundlage, `api/learn/cloze-texts`): Text mit `{{1}}`-Platzhaltern, `Gaps` (Lösung + Alternativen), optionaler `WordBank` und Übersetzung – Gaps/WordBank als JSON-Spalten, eindeutiger `Key`, Löschschutz solange in einem Plan verwendet.
...
**Umgesetzt:**
- **Login** (`/api/auth/father`, `/api/auth/child`) per Id + PIN → signiertes JWT (`TokenService`, `JsonWebTokenHandler`, HS256, 12 h). Claims: Rolle, `fid`/`cid`, Name. `Child.Pin` ergänzt.
...
- **`PracticeSessionsController.Review`** schreibt bei Leitner-Plänen die Box fort **und bucht Punkte ins Kind-Ledger** (`ChildPointsEntry`); Rückgabe `{ awarded, box, dueOn }`. Nicht-Leitner-Pläne liefern weiter `204` (kein Regress).
