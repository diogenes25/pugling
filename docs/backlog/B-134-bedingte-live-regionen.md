...
# B-134 · Zwölf Live-Regionen entstehen zusammen mit ihrem Text — und schweigen darum
...
Der Sweep über neun Dateien und ein Wächter darüber sind eine andere Arbeit als jener Fix — deshalb eine
...
einzeln angesehen — nicht hochgerechnet): **zwölf Regionen stehen hinter einer Bedingung**, in neun
Dateien.
...
Nicht „Barrierefreiheit wurde vergessen" — das Gegenteil: an allen zwölf Stellen hat jemand `role`
...
2. **Ist `role="status"` überall überhaupt richtig?** Mindestens zwei Fälle sind zweifelhaft:
   ...
   umzuformen — sonst wird aus zwölf richtigen Attributen zwölf falsche Dauer-Regionen.
3. **Reicht `StatusBanner` als Ersatz für die meisten?** Sieben der zwölf sind schlichte
   Banner/Toasts, für die es die Komponente längst gibt. Empfehlung: dort ersetzen statt reparieren —
   das entfernt die Fehlerquelle, statt sie an zwölf Stellen einzeln zu umgehen. Kosten: die Optik der
   Toasts (`className="toast"` in der Sohn-Arcade) unterscheidet sich vom Banner; das muss angesehen
   werden, bevor es getauscht wird.

## Akzeptanzkriterien

1. Jede der zwölf Fundstellen ist entweder auf eine dauerhaft gerenderte Region umgestellt, durch
