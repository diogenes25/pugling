...
### 1.1 Anmelden
```
POST /api/auth/father   { "fatherId": 1, "pin": "0000" }
→ { token, role:"Vater", ... }
```
...
```
POST /api/fathers/1/children        { "name":"Peter", "birthYear":2015, "pin":"1111" }
GET  /api/fathers/1/children        → Liste inkl. Punktestand
```

### 1.3 Lerngrundlagen anlegen
**Vokabeln** (Basis für Vokabel- und Matching-Verfahren):
```
POST /api/learn/vocabulary
...
Weitere Vokabel/Text nachträglich: `POST /api/study-plans/{id}/items { "contentKey":"…" }`.
Regeln ändern: `PATCH /api/study-plans/{id}`.
...
```
POST /api/children/{childId}/timetable   { "subjectId": 1, "dayOfWeek":"Tuesday", "timeOfDay":"Nachmittag" }
...
```
GET /api/study-plans/{id}/today     → heute Pflicht erfüllt? empfohlene Stufe, offene Punkte, Streak, schwache Inhalte
GET /api/study-plans/{id}/progress  → Tag-für-Tag: Minuten, bester Test, komplett?, Punkte
GET /api/study-plans/{id}/report    → pro Inhalt: Wiederholungen, Test-Treffer, Mastery% + Testhistorie
GET /api/fathers/1/children/1/points → Punkte-Saldo + Buchungen (Ledger)
```
Manuelle Punktekorrektur / Belohnung:
```
POST /api/fathers/1/children/1/points   { "amount": 30, "reason":"Extra fürs Dranbleiben" }
```
...
### 2.1 Anmelden
```
POST /api/auth/child   { "childId": 1, "pin": "1111" }   → token (Rolle "Sohn")
```

### 2.2 Was ist heute zu tun?
```
GET /api/study-plans?childId=1        → meine Pläne
GET /api/study-plans/{id}/today       → dutyDone, offene Pflichten, empfohlene Stufe, schwache Inhalte
```
...
**Vokabeltest:**
```
POST /api/study-plans/{id}/tests            { }             → attemptId + Karten (ohne Lösung)
POST /api/study-plans/{id}/tests/{aid}/hint { "vocabularyId":X }   (Stufe 3+: deckt einen Buchstaben auf)
POST /api/study-plans/{id}/tests/{aid}/submit
...
**Lückentext-Test:**
```
POST /api/study-plans/{id}/cloze-tests             { }   → Texte mit Lücken + (je Stufe) Übersetzung/Wortpool
POST /api/study-plans/{id}/cloze-tests/{aid}/hint  { "clozeTextId":X,"gapIndex":1 }   (nur Freitext-Stufen)
POST /api/study-plans/{id}/cloze-tests/{aid}/submit
   { "answers":[ {"clozeTextId":X,"gapIndex":1,"givenAnswer":"Hello"} ] }
```

**Matching-Test:**
```
POST /api/study-plans/{id}/matching-tests             { }   → items (Prompts) + options (gemischter Pool)
POST /api/study-plans/{id}/matching-tests/{aid}/submit
   { "answers":[ {"vocabularyId":X,"chosenAnswer":"Hund"} ] }
```

### 2.5 Belohnung
Punktestand: `GET /api/fathers/1/children/1/points` (oder im `today`/Report). Beim Bestehen und beim
...
```
POST /api/study-plans/{id}/ratings
