    ...
    Services.Shared.PositionProgressService progress, Services.Shared.ObjectiveRewardService objectiveRewards) : ControllerBase
{
    ...
    /// </para>
    /// <para>
    /// Written to <b>two</b> places, because identity hangs off two: the <see cref="Account"/>
    /// carries the login, the <see cref="Adult"/> row the domain name (it appears as the author on
    /// exercises). The PIN hash must be mirrored regardless, otherwise the account-centric login falls out
    /// of sync; name and email are mirrored here <i>as well</i> – <c>AdultsController</c> doesn't do that,
    /// which is why account name and adult name could otherwise drift apart.
    /// </para>
    /// <para>
