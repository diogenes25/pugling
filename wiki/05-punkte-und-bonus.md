...
| `GoalPenalty` | 🪙 Münzen | **Pflicht-Malus** – gerissenes Positionsziel (negativ, siehe §2) |
...
| `ManualGems` | 💎 Gems | manuelle Vater-Buchung in Gems (**Gem-Geschenk**, siehe §7) |
...
### Der Pflicht-Malus (der „Stick") — `PositionProgressService.SettleClosedPeriodsAsync`

Bahn A/B sind reine Belohnung (Carrots). Der **Malus** ist die Gegenkraft und setzt das Kern-Versprechen
der App um: *Lernen wird erzwungen.* Reißt eine Pflichtposition (`cadence=Daily|Weekly`) mit gesetztem
**`penaltyCoins`** ihre **abgeschlossene** Periode (Ziel nicht erreicht), werden `penaltyCoins` 🪙 als
`PointKind.GoalPenalty` **abgezogen**.

- **Schulden erlaubt:** der Münz-Saldo darf dadurch **negativ** werden und blockt dann den Shop-Kauf
  (`insufficient_coins`), bis das Kind wieder verdient **oder der Vater Münzen verschenkt** (§7).
- **Idempotent** je Position/Periode über `PositionGoalPenalty` (Unique-Index `(PlanPositionId, PeriodKey)`,
  das negative Spiegelbild von `PositionGoalReward` → nie doppelt bestraft).
- **Kein Scheduler:** die App rechnet **faul** an POST-Nahtstellen nach — beim **Kind-Login** (`auth/child`)
  und beim **Shop-Kauf** (vor der Deckungsprüfung, also nicht durch „schnell kaufen" umgehbar).
  Rückblick höchstens 14 Tage.
- **Fairness:** kein Malus für Perioden, in denen der Plan **inaktiv** oder **außerhalb des Datumsfensters**
  war (der Vater hatte ihn aus → es durfte gar nicht gelernt werden).
- Default `penaltyCoins=0` → reine Belohnung (bisheriges Verhalten). Der Malus ist **opt-in pro Position**.

Der Malus wohnt bewusst am **Pflichtziel (Münzen = reale Privilegien)** und **nicht** am Missions-System
(§5, Gems = Kosmetik) — nur so trifft die Konsequenz spürbar.
...
1. **Pro Position** — die Bonus-Felder (`comboThreshold`, `comboBonusPoints`, `speedThresholdSeconds`,
   `speedBonusPoints`, `newContentPoints`, `pointsGoalMet`) sowie der **Münz-Malus** `penaltyCoins`
   (0 = aus, siehe §2) jederzeit per
...
### Verschenken (Vater → Kind, 🪙 Münzen **oder** 💎 Gems)

Die manuelle Vater-Buchung `POST /api/v1/supervisor/children/{childId}/points` nimmt neben `amount`
(positiv = gutschreiben/verschenken, negativ = abziehen) und `reason` jetzt eine **`currency`**:

```http
POST /api/v1/supervisor/children/{childId}/points
{ "amount": 50, "reason": "Zimmer aufgeräumt", "currency": "Coins" }   // → PointKind.Manual
{ "amount": 20, "reason": "Extra fürs Vorlesen", "currency": "Gems" }  // → PointKind.ManualGems
```

So kann der Vater auch **Gems verschenken** (ging vorher nicht — Münzen konnte er schon immer manuell buchen).
Zweck: **Belohnung außerhalb der App** *und* das **Druckventil gegen die Malus-Schulden-Todesspirale** (§2) —
zu hohe Schulden töten die Motivation, also kann der Vater sie gezielt ausgleichen. Im Frontend sitzt das als
„Verschenken"-Dialog unter `/vater/konto` (`VaterKonto.tsx`), inkl. Hinweis bei negativem Münzsaldo.

> **Merke:** Der Familien-Shop bleibt der einzige Weg, auf dem der **Sohn** Münzen *ausgibt*. Daneben gibt es
> zwei vater-getriebene Münz-Bewegungen: der **Malus** zieht Münzen ab (§2), das **Verschenken** gibt welche dazu.
