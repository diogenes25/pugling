...
code=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/api/v1/supervisor/study-plans/999/positions")
...
code=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/api/v1/supervisor/study-plans/999/positions" "${AUTH[@]}")
...
STORE=$(curl -s "$BASE/api/v1/creator/vocabulary?take=2" "${AUTH[@]}")
...
PID=$(curl -s -X POST "$BASE/api/v1/supervisor/study-plans" "${AUTH[@]}" -H "Content-Type: application/json" \
  -d '{"childId":1,"title":"Smoke Plan","durationDays":5}' | jget id)
[ -n "$PID" ] && pass "Plan angelegt (id=$PID)" || bad "Plan anlegen fehlgeschlagen"
POS=$(curl -s -X POST "$BASE/api/v1/supervisor/study-plans/$PID/positions" "${AUTH[@]}" -H "Content-Type: application/json" \
...
if [ -n "${POS:-}" ]; then
  ATT=$(curl -s -X POST "$BASE/api/v1/student/study-plans/$PID/positions/$POS/tests" "${AUTH[@]}" -H "Content-Type: application/json" -d '{"stage":2}')
  ...
  RES=$(curl -s -X POST "$BASE/api/v1/student/study-plans/$PID/positions/$POS/tests/$AID/submit" "${AUTH[@]}" -H "Content-Type: application/json" -d "{\"answers\":[$ans]}")
