...
code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/study-plans/999/tests" -H "Content-Type: application/json" -d '{}')
[ "$code" = "401" ] && pass "401 ohne Token" || bad "erwartete 401, war $code"

# 2) Vater-Login liefert Token
TOK=$(curl -s -X POST "$BASE/api/auth/father" -H "Content-Type: application/json" -d '{"fatherId":1,"pin":"0000"}' \
...
code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/study-plans/999/tests" "${AUTH[@]}" -H "Content-Type: application/json" -d '{}')
...
PLAN=$(curl -s -X POST "$BASE/api/study-plans" "${AUTH[@]}" -H "Content-Type: application/json" \
...
if [ -n "${PID:-}" ]; then
  ATT=$(curl -s -X POST "$BASE/api/study-plans/$PID/tests" "${AUTH[@]}" -H "Content-Type: application/json" -d '{"stage":"SelfAssess"}')
  ...
  RES=$(curl -s -X POST "$BASE/api/study-plans/$PID/tests/$AID/submit" "${AUTH[@]}" -H "Content-Type: application/json" -d "{\"answers\":[$ans]}")
