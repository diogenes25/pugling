...
# 5) Zwei Vokabel-Keys aus dem geseedeten Store holen (robust statt hartkodiert)
KEYS=$(curl -s "$BASE/api/v1/learn/vocabulary?take=2" "${AUTH[@]}" \
  | python -c "import sys,json;print(' '.join(v['key'] for v in json.load(sys.stdin)[:2]))" 2>/dev/null)
[ -n "$KEYS" ] && pass "Vokabel-Store liefert Keys ($KEYS)" || bad "keine Vokabeln im Store"
KEYS_JSON=$(python -c "import sys,json;print(json.dumps(sys.argv[1:]))" $KEYS)

# 6) Fach + Kapitel + Vokabelübung (referenziert die Store-Keys) anlegen
...
EX=$(curl -s -X POST "$BASE/api/v1/learn/subjects/$SUBJ/chapters/$CHAP/vocabulary" "${AUTH[@]}" -H "Content-Type: application/json" \
  -d "{\"title\":\"Smoke-Uebung\",\"orderIndex\":1,\"rewardPoints\":10,\"config\":{\"direction\":\"front-to-back\",\"refs\":$KEYS_JSON}}" | jget id)
...
if [ -n "${POS:-}" ]; then
  ...
  ans=$(echo "$ATT" | python -c "import sys,json;d=json.load(sys.stdin);print(','.join('{\"itemIndex\":%d,\"wasKnown\":true}'%i['itemIndex'] for i in d.get('items',[])))" 2>/dev/null)
